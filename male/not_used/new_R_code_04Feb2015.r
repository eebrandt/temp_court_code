...
# CLEAN DATA
# assigns nothing to a few columns that we aren't interested in
dat$tape.AF8.video <- NULL
dat$comments <- NULL
dat <- dat[which(!dat$complete=="NA"), ]

# assigns names to our columns
colnames(dat) <- c(
 "complete",
 "ind",
 "treatment",
 ...
 "bfreq"
  ...
  # makes individual a factor (I think this is automatically done since some of them have letters)
...
# not entirely sure what this section does...
...
#predfit.list <- list() # creates list to hold predicted values
...
for (i in seq_along(mod.list)) {
 ...
 #as.numeric(rownames(newdat))
 #predfit.list[i] <- list(predfit)
