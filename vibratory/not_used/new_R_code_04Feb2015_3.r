...
meanlistQ10 <- vector(mode = "numeric")
spotlistQ10 <- vector(mode = "numeric")
par(mfrow=c(2,5))
for (i in seq_along(mod.list)) {
 ...
 # calculate Q10 stuff here 
 # Line from which we'll take our Q10 data
 ...
 for (j in 1:268){
   Q10 <- predfit[j+10]/predfit[j]
   Q10calc <-  c(Q10calc, Q10)
   Q10 <- predfit[279 - j]/predfit[279 - (j + 10)]
 ...
 meanlistQ10 <- c(meanlistQ10, mean(Q10calc))
 spotlistQ10 <- c(spotlistQ10, Q10calc["151"]) 
}
names(spotlistQ10) <- c("scrape", "srms", "thump", "trms", "buzz", "brms", "srates", "sfreq", "tfreq", "bfreq")
names(meanlistQ10) <- c("scrape", "srms", "thump", "trms", "buzz", "brms", "srates", "sfreq", "tfreq", "bfreq")
